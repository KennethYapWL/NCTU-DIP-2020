...
% 3. LoG Filter
edges = laplacian_gaussian_filter(grayS, 2.2);
figIdx = figIdx + 1;
figure(figIdx);
imshow(edges);
...
tmp = edges;
edges = convert_to_binary_image(edges, 0.35);
figIdx = figIdx + 1;
figure(figIdx);
imshow(edges);
...
% 3. LoG Filter
edges = laplacian_gaussian_filter(grayS_pre, 2.5);
figIdx = figIdx + 1;
figure(figIdx);
imshow(edges);
title('LoG Filtering Result');
...
edges = canny_edge_detection(grayS_pre, 0.09, 30, 60);
figIdx = figIdx + 1;
figure(figIdx);
imshow(edges);
title('Canny Edge Detection Result (with preprocessing)');
